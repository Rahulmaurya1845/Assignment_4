...
function AllStaff() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      .then((res) => {
        ...
        // Filter only male staff
        const maleStaff = capitalizedData.filter(
          (teacher) => teacher.gender.toLowerCase() === "male"
        );

        setstaff(maleStaff);
        setstoreData(maleStaff);
      })
      .catch((err) => {
        setloading(false);
      });
  ...
  const generatePDF = () => {
    ...
    pdf({ data: staff, headers, filename: "MaleStaff" });
  };
  ...
  const handleReset = (e) => {
    e.preventDefault();
    setstaff(storeData); // Reset to original male staff only
    setname("");
    setuserID("");
  };
  ...
  return (
    <>
      ...
      <StaffTable
        route="staff"
        loading={loading}
        noData="No male staff members yet"
        students={staff}
