...
function AllStudents() {
 ...
 axios.get("/students")
  .then((res) => {
    ...
    const femaleStudents = processed.filter(s => s.gender === "female");
    console.log("Filtered female students:", femaleStudents); 

    setStudents(femaleStudents);

    // 5. Fetch dormitory data only if valid data is not already in local storage
    if (!dormitoryExists) {
      const dormitoryPromises = femaleStudents.map(student =>
