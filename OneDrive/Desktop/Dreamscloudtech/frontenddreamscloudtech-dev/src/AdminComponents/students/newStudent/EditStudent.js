...
function EditStudent() {
  ...
  useEffect(() => {
    axios.get(`/students/student/${id}`).then((res) => {
      ...
      setdisease(data?.disease ? moment(data?.disease).format("YYYY-MM-DD") : " ");
      ...
      // set fathers info
      console.log(data?.guadian);

      const father = data?.guadian.find((item) => item.relationship === "Father");
      console.log(father, "father");
      
      setfatherName(father?.name||"");
      setfatherLastName(father?.lastname||"");
      setfatherMobile(father?.mobile||"");
      setfatherEmail(father?.email||"");
      setfatherOccupation(father?.occupation||"");
      setfatherAddress(father?.address||"");
      // set mothers info
      const mother = data?.guadian.find((item) => item.relationship === "Mother");
      setmotherName(mother?.name||"");
      setmotherLastName(mother?.lastname||"");
      setmotherMobile(mother?.mobile||"");
      setmotherEmail(mother?.email||"");
      setmotherOccupation(mother?.occupation||"");
      setmotherAddress(mother?.address||"");
    });
  }, []);
      ...
      <div style={{ backgroundColor: "#EEF7FF" }}>
        ...
        >
          ...
          <div className="row ">
            ...
            <button onClick={handleReset} className=" col btn red__btn mr-5">
              Reset
            </button>
          </div>
        </form>
