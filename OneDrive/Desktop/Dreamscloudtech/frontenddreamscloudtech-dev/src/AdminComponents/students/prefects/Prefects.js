...
const tableHeader = [
  { id: "userID", name: "Student" + String.fromCharCode(160) + "ID" },  // Non-breaking space to keep "Student ID" in one line without wrapping
  { id: "name", name: "Name" },
  { id: "position", name: "Position" },
  { id: "startYear", name: "From" },
  { id: "endYear", name: "Till" },
...
function Prefects() {
  ...
  const onEdit = () => {
    ...
    axios
      .put(`/prefects/update/${edituserID}`, {
        name: editname,
        position: editposition,  // Fixed typo: "posistion" to "position"
        endYear: editendYear,
        userID: edituserID,
      })
      .then(async (res) => {
        ...
        setprefects([res.data.doc, ...filteredData]);
        await axios.post("/activitylog/create", {
          activity: `prefect  ${editname} was edited`,  // Fixed: use editname instead of name
          user: "admin",
        });
      })
      .catch((err) => {
      ...
      <div className="row">
        <div className="col-sm-12 col-md-5">
          <AddPrefect
            ...
            setuserID={setuserID}
            handleAdd={handleAdd}
            posistion={position}  // Note: This prop is misspelled in AddPrefect component; should be "position"
            setposition={setposition}
          />
        </div>
        <div className="col-sm-12 col-md-7" style={{ padding: '10px' }}>  {/* Added padding to create space from borders */}
          <PrefectsList
            handleEdit={handleedit}
            loading={dataloading}
            noData="No prefects yet"
