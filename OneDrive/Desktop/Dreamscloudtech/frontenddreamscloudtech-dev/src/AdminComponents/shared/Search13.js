...
function Search(props) {
  ...
  return (
    <form className="mb-0 content__container" onSubmit={(e) => e.preventDefault()} style={{ backgroundColor: "#EEF7FF" }}>
      <h3 className="mb-3">{title || ""}</h3>

      <div className="row g-3 mb-3 align-items-end">
        {/* Register Attendance Button */}
        <div className="col-xs-12 col-sm-6 col-md-3">
          <Link to={`/academics/classes/attendance/record/${id}`} className="btn green__btn w-90" style={{ fontSize: "13px", marginBottom: "6px" }}>
            Register Today's Attendance
          </Link>
        </div>
        <div className="col-xs-12 col-sm-6 col-md-2">
          <Link to={`/academics/classes/${id}`} className="btn blue__btn w-90" style={{ fontSize: "13px", marginBottom: "6px" }}>
            Class Details
          </Link>
        </div>
        {/* Input Fields */}
